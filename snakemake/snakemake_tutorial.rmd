...
## The config file
The way our pipeline is currently written, if you want to change the samples, you need to open the snakefile and find where that is defined. This can be a bit tricky, and you don't necessarily want people directly modifying the Snakemake file. A great alternative to this is the addition of a `configfile`. This is a file written in `yaml` format that can be modified by the user and then read into snakemake. The config file can contain anything you want. I most often use it for sample names and paths to files (like the genome reference for alignment). I sometimes also use it for parameters that may be changed (like the adaptor sequences for trimming).
...
Again, I won't show you all here, not notice that the wildcard is now `sample_5` which came from the `config.yaml` document.  Also, because I just specified `results/{sample}_params_ex.txt` Snakemake checks for each of the necessary input files and makes all files necessasry (all of them for samples 5-8)
...
rule python_example:
    ...
    output:
        python_ouput = "results/{sample}_python_code.txt",
...
rule python_example:
    ...
    run:
        ...
        with open(output.python_ouput, "w") as out_file:
...
rule python_script:
    ...
    output:
        python_ouput = "results/{sample}_python_script.txt"
    params:
        sample_list = SAMPLES_TWO
    script:
        "src/scripts/python_example.py"
```
